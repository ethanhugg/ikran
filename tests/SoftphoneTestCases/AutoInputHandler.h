...
public:
	...
	vector<eUserOperationRequest>* GetActionVector(){
		return &actionVector;
	}
...
private:
	...
	vector<eUserOperationRequest> actionVector;

	class UserInputWorkItem : public base::SimpleThread
	{
	public:
		UserInputWorkItem (AutoInputHandler* context) : base::SimpleThread("UserInputWorkItem"), context(context) {};
		~UserInputWorkItem();
		virtual void Run();

	private:
		AutoInputHandler* context;
		void signalSendDTMF (int asciiChar);
		std::string getInput(const std::string& prompt);
	};

	UserInputWorkItem thread;

...
#if ENABLE(SIP)
void WebMediaPlayerClientImpl::sipRegStateChanged()
{
    ASSERT(m_mediaPlayer);
    m_mediaPlayer->sipRegStateChanged();
}

void WebMediaPlayerClientImpl::sipSessionStateChanged()
{
    ASSERT(m_mediaPlayer);
    m_mediaPlayer->sipSessionStateChanged();
}
#endif
...
#if ENABLE(SIP)
void WebMediaPlayerClientImpl::load(const String& url,const String src, const String aor, const String credentials, const String proxy, const String dn)
{
	m_url = url;
    Frame* frame = static_cast<HTMLMediaElement*>(
        ...
        LOG(Media,"  SOMERHING FISHY ");
        LOG(Media, " Aor is %s",aor.utf8().data());
        LOG(Media, " src is %s",src.utf8().data());
        LOG(Media, " creds is %s",credentials.utf8().data());
        LOG(Media, " proxy is %s",proxy.utf8().data());
        LOG(Media, " DN is %s",dn.utf8().data());

    if (m_webMediaPlayer)
        m_webMediaPlayer->load(KURL(ParsedURLString, m_url),WebCString(src.ascii().data(),src.ascii().length()), WebCString(aor.ascii().data(),aor.ascii().length()), WebCString(credentials.ascii().data(),credentials.ascii().length()),WebCString(proxy.ascii().data(),proxy.ascii().length()), WebCString(dn.ascii().data(), dn.ascii().length()));
}
#endif
...
#if ENABLE(SIP)
void WebMediaPlayerClientImpl::openSession(const String dn)
{
//        m_webMediaPlayer->openSession();
 if (m_webMediaPlayer.get())
                m_webMediaPlayer->openSession(WebCString(dn.ascii().data(),dn.ascii().length()));

}

void WebMediaPlayerClientImpl::closeSession()
{
    if (m_webMediaPlayer.get())
        m_webMediaPlayer->closeSession();
}

void WebMediaPlayerClientImpl::clearSession()
{
    if (m_webMediaPlayer.get())
        m_webMediaPlayer->clearSession();
}
...
MediaPlayer::SipRegistrationState WebMediaPlayerClientImpl::sipRegistrationState() const
{
    if (m_webMediaPlayer.get())
        return static_cast<MediaPlayer::SipRegistrationState>(m_webMediaPlayer->sipRegistrationState());
    return MediaPlayer::NoRegistrar;
}

MediaPlayer::SipSessionState WebMediaPlayerClientImpl::sipSessionState() const
{
    if (m_webMediaPlayer.get())
        return static_cast<MediaPlayer::SipSessionState>(m_webMediaPlayer->sipSessionState());
    return MediaPlayer::NoSession;
}

WTF::String WebMediaPlayerClientImpl::callingPartyName() const
{

    if (m_webMediaPlayer.get())
    {
                LOG(Media," WebMediaPlayerClientImpl:: Name :: %s", m_webMediaPlayer->callingPartyName().data());
                WTF::String callerName(m_webMediaPlayer->callingPartyName().data());
                return callerName;
        }
    return "invalid";
}

WTF::String WebMediaPlayerClientImpl::callingPartyNumber() const
{

    if (m_webMediaPlayer.get())
    {
        WTF::String callerNumber(m_webMediaPlayer->callingPartyNumber().data());
        return callerNumber;
    }
    return "invalid";
}

...
scoped_refptr<media::VideoDecoder> MediaStreamImpl::GetVideoDecoder(
  ...
  DCHECK(CalledOnValidThread());
  LOG(INFO) << " Suhas:GetVideoDecoder called  with IsLocal (??) " << isLocal;
  scoped_refptr<media::VideoDecoder> decoder;

#if defined(_ENABLE_SIP_)
	if(isLocal == true)
	{
  		LOG(INFO) << " GetVideoDecoder : Setting up Local Capture Renderer ";
    	sipcc_capture_decoder_ = new SIPCCVideoDecoder(
       			 					message_loop_factory->GetMessageLoop("SipccCaptureDecoderThread"),
        		 					url.spec());
    	decoder = sipcc_capture_decoder_;
    	LOG(INFO) << "Suhas: RTCVideoDecoder: Setting capture video renderer as " << sipcc_capture_decoder_;
    	sipcc_renderer->SetCaptureVideoRenderer(sipcc_capture_decoder_);
    	sipcc_renderer->SetCaptureDecoderMessageLoop(sipcc_capture_decoder_->MyMessageLoop());
	} else {
  		LOG(INFO) << " GetVideoDecoder : Setting up Remote Renderer ";
    	sipcc_receive_decoder_ = new SIPCCVideoDecoder(
       			 					message_loop_factory->GetMessageLoop("SipccReceiveDecoderThread"),
        		 					url.spec());
    	decoder = sipcc_receive_decoder_;
    	LOG(INFO) << "Suhas: RTCVideoDecoder: Setting receive video renderer as " << sipcc_receive_decoder_;
    	sipcc_renderer->SetReceiveVideoRenderer(sipcc_receive_decoder_);
    	sipcc_renderer->SetReceiveDecoderMessageLoop(sipcc_receive_decoder_->MyMessageLoop());
	}
  return decoder;
#else 
  ...
  if (label.empty())
    return NULL;
  LOG(INFO) << " GetVideoDecoder: Label string is " << label;
  if (media_stream_dispatcher_->IsStream(label)) {
    LOG(INFO) << " GetVideoDecoder:  Handling local stream " ;
    // It's a local stream.
    ...
    LOG(INFO) << " GetVideoDecoder:  Handling remote stream " ;

  ...
  DSCPValue(0)
{
   ...
   //let's try enabling traces
   error =	vieVideo->SetTraceFilter(webrtc::kTraceAll);
   if(error == -1)
   {
		LOG_WEBRTC_ERROR( logTag, "WebrtcVideoProvider(): VieVideoEngine::SetTraceFilter() failed : %d ", vieBase->LastError());
   }
	error = 0;
    error = vieVideo->SetTraceFile( "/Users/XTM/Vievideotrace.out" );
   if(error == -1)
   {
		LOG_WEBRTC_ERROR( logTag, "WebrtcVideoProvider(): VieVideoEngine::SetTraceFile() failed : %d ", vieBase->LastError());
   }
												...
												ExternalRendererHandle renderer)
{
	...
	if(channel != -1)
	{
		...
		error = vieRender->AddRenderer(channel, (webrtc::RawVideoType)webrtc::kVideoI420, (webrtc::ExternalRenderer*) renderer); 
		if(error == -1)
		{

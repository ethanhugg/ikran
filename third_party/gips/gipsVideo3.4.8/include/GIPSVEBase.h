...
public:
    ...
    virtual void CallbackOnTrace(const GIPS::TraceLevel level, const char* message, const int length) = 0;
protected:
...
public:
    ...
    virtual int GIPSVE_SetObserver(GIPSVoiceEngineObserver& observer, bool clear = false, bool excludeTrace = false) = 0;
    ...
    // Trace functions
    virtual int GIPSVE_SetTraceFileName(const char* fileNameUTF8, bool newFileOnWrap = false) = 0; 
    virtual int GIPSVE_SetDebugTraceFileName(const char* fileNameUTF8, bool newFileOnWrap = false) = 0; 
    virtual int GIPSVE_SetTraceFilter(unsigned int filter) = 0;

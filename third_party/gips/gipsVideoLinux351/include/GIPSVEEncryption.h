...
public:
    ...
    // SRTP
    virtual int GIPSVE_EnableSRTPSend(int channel, GIPS_CipherTypes cipherType, unsigned int cipherKeyLength,
        GIPS_AuthenticationTypes authType, unsigned int authKeyLength, unsigned int authTagLength,
        GIPS_SecurityLevels level, const unsigned char* key, bool useForRTCP = false) = 0;
    virtual int GIPSVE_DisableSRTPSend(int channel) = 0;
    virtual int GIPSVE_EnableSRTPReceive(int channel, GIPS_CipherTypes cipherType, unsigned int cipherKeyLength,
        GIPS_AuthenticationTypes authType, unsigned int authKeyLength, unsigned int authTagLength,
        GIPS_SecurityLevels level, const unsigned char* key, bool useForRTCP = false) = 0;

...
{
    //bind_show_keyword("cpr-memory", cpr_show_memory);
    //bind_clear_keyword("cpr-memory", cpr_clear_memory);
    //bind_debug_func_keyword("cpr-memory", cpr_debug_memory_cli);
 ...
 * @return none
 *
 * @pre (cpr_mem not_eq NULL)
 * @pre ((size >> 31) equals 0)
 *
 ...
 *
 * @pre (cpr_mem not_eq NULL)
 * @pre (cpr_mem->u.node not_eq NULL)
 *
 ...
 * @return none
 * 
 * @pre (memory_tracking.enabled equals TRUE)
 * @pre (cpr_mem not_eq NULL) and (cpr_mem->pc not_eq NULL)
 * @pre (size not_eq 0)
 */
static void
cpr_assign_mem_tracking_node (cpr_mem_t *cpr_mem, size_t size)
 ...
 * @pre (cpr_mem not_eq NULL)
 * @pre ((size >> 31) equals 0)
 * @pre (((cpr_mem->u.size bitand 0x1) equals 0) or
 ...
 *       have been parsed, i.e. when debug equals FALSE all arguments
 *       are ignored.
 */
static boolean
cpr_set_memory_tracking (boolean debug, int32_t argc, const char *argv[])

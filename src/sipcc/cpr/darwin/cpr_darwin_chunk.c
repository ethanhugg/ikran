 ...
 *
 * @see cpr_darwin_chunk.c Detailed Description for general description
 *
 * @{
 */
 ...
 *
 * @pre (overhead not_eq NULL)
 * @pre (req_memsize not_eq NULL)
 *
...
data_to_chunk_hdr (chunk_header_t *p)
{
    ...
    return (chunk_header_t *)
        (p - FIELDOFFSET(chunk_header_t, data_area.data));
}
...
chunk_populate_freelist (chunk_t *chunk, uint32_t real_size)
{
    ...
    for (count = 0; count < chunk->real_max; count++) {
        ...
        if (alignment) {
            ...
            data_alignment = ((long)freehdr % alignment);
            if (data_alignment) {
                 ...
                 */
                data = (void *)((long)data + data_alignment);
            }
        }
        hdr = (chunk_header_t *) data;
        freehdr = hdr_to_free_chunk(hdr);
              ...
              const char *name)
{
    ...
    uint32_t caller_pc = (long) CHUNK_CALLER_PC;
 ...
 * @return none
 *
 * @pre (head_chunk not_eq NULL)
 * @pre (chunk not_eq NULL)
 *
...
poison_chunk (chunk_t *chunk, void *data, uint32_t size)
{
    ...
    data2 = (uint32_t *)((long)data + sizeof(free_chunk_t));
    size -= sizeof(free_chunk_t);
                 ...
                 uint32_t caller_pc)
{
    ...
    ((free_chunk_t *)data)->last_deallocator = (void *)(long)caller_pc;

...
#ifdef NOT_USED
char
pmhutils_rstream_read_byte (pmhRstream_t *pmhRstream)
{
    char ret;
...
#ifdef NOT_USED
pmhTokens_t *
pmh_tokenize (const char *str, const char *tstr)
{
    uint16_t     num_tokens = 0;
    uint16_t     max_tokens = 10;
    pmhTokens_t *tokens = NULL;

    if (str && tstr && str[0] && tstr[0]) {
        char *mystr;

        mystr = cpr_strdup(str);
        if (!mystr) {
            return (NULL);
        }

        tokens = (pmhTokens_t *) cpr_malloc(sizeof(pmhTokens_t));
        if (!tokens) {
            cpr_free(mystr);
            return (NULL);
        }

        tokens->tokens = (char **) cpr_malloc(max_tokens * sizeof(char *));
        if (!tokens->tokens) {
            cpr_free(mystr);
            cpr_free(tokens);
            return (NULL);
        ...
        tokens->num_tokens = 0;
        if (tokens && tokens->tokens && mystr) {
            char *this_token = strtok(mystr, tstr);

            while (this_token) {
                tokens->tokens[num_tokens] = cpr_strdup(this_token);

                if (!tokens->tokens[num_tokens]) {
                    cpr_free(mystr);
                    pmh_tokens_free(tokens);
                    cpr_free(tokens);
                    return (NULL);
                }
                ...
                if (num_tokens >= max_tokens) {
                    tokens->tokens = cpr_realloc(tokens->tokens,
                                                 (max_tokens + 10) * sizeof(char *));
                    max_tokens += 10;
                    /* Realloc and check for sanity */
                    if (!tokens->tokens || (max_tokens > 100)) {
                        UTILFREE(mystr);
                        pmh_tokens_free(tokens);
                        UTILFREE(tokens);
                        return (NULL);
...
#ifdef NOT_USED
void
pmh_tokens_free (pmhTokens_t *tokens)
{
    if (tokens && tokens->tokens) {
        uint16_t i = 0;

        while (i < tokens->num_tokens) {
            UTILFREE(tokens->tokens[i]);
            i++;
        }
        UTILFREE(tokens->tokens);
        tokens->num_tokens = 0;
    }
}
#endif
